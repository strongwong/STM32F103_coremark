...
volatile float CoreMark_num = 0.0;
volatile float CoreMark_time = 0.0;
volatile float CoreMark_Iterations = 0.0;
...
#error "Please define a way to initialize a memory block."
	...
	CoreMark_time = total_time;
#if HAS_FLOAT
	ee_printf("Total time (secs): %f\n",time_in_secs(total_time));
	CoreMark_time = time_in_secs(total_time);
	...
	CoreMark_Iterations = (default_num_contexts*results[0].iterations);
	...
	if (total_errors==0) {
		...
		if (known_id==3) {
			...
			CoreMark_num = (default_num_contexts*results[0].iterations/time_in_secs(total_time));
